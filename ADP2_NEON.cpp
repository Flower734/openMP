...
vector<unsigned int>& ADP(int* list, vector<index>& idx, int num)
{
	...
	#pragma omp parallel num_threads(NUM_THREADS)\
     shared(s,idx_,num,list,ans),private(t_id,i,j,m,t)
    #pragma omp for
    for ( t_id = 0; t_id < NUM_THREADS; t_id++) {
        ...
        for( j=1;j<num;j++){
			...
			uint32_t length = ceil(s0[t_id].len / 4) * 4;
			for ( m = s0[t_id].len ; m < length; m++)
				s0[t_id].arr[m] = 0;
            //从第1个数组开始遍历，如果第一个中就没有，则退出循环
			for ( t = 0; t < length; t+=4 ) {//��i���б��ĸ���ֵ��s�б��еĵڡ�j]��ֵ�Ƚ�
				found = true;
				unsigned int res[4] = { 0 };
				uint32x4_t temp0 = vmovq_n_u32(e);
				uint32x4_t temp1 = vld1q_u32( &s0[t_id].arr[t]);
